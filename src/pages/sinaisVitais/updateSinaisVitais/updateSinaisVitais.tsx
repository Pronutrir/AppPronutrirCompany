...
type ProfileScreenRouteProp = RouteProp<RootStackParamList, 'UpdateSinaisVitais'>;
interface Props {
  route: ProfileScreenRouteProp;
}

const UpdateSinaisVitais: React.FC<Props> = ({
  route: {
    params: { AgendaPaciente, SinaisVitais, GeraAtendimento, Origin },
  },
}: Props) => {
  ...
  const { data: historicoSinaisVitais } = useSinaisVitaisHistory({
    nomePaciente: AgendaPaciente.nM_PESSOA_FISICA,
  });
  ...
  const SinaisVitaisUpdate = async () => {
    ...
    await UpdateSinaisVitais({
      cD_PACIENTE: AgendaPaciente.cD_PESSOA_FISICA,
  ...
  const PostSinaisVitais = async () => {
    ...
    const dataSinaisVitais: SinaisVitaisPost = {
      ...
      cD_ESTABELECIMENTO: AgendaPaciente.cD_ESTABELECIMENTO,
      cD_MEDICO_RESP: AgendaPaciente.cD_MEDICO ?? null,
      nM_PESSOA_FISICA: AgendaPaciente.nM_PESSOA_FISICA,
      dT_NASCIMENTO: AgendaPaciente.dT_NASCIMENTO
    };
  ...
  const gerarSenha = async (seqFila: number) => {
    ...
    const { cD_ESTABELECIMENTO, cD_PESSOA_FISICA } = AgendaPaciente;
    ...
    try {
      ...
      await printSenha(result, AgendaPaciente.nM_PESSOA_FISICA);
    } catch (error) {
      loadingRef.current?.closeModal();
    } finally {
      loadingRef.current?.closeModal();
  ...
  const MenuPopUpOptions = async (itemSelected: string) => {
    switch (itemSelected) {
      case 'Gerar senha': Origin === 'Tratamento' ? gerarSenha(1) : refModalCentralizeSenha.current?.openModal()
        break;
      case 'Histórico':
        navigation.navigate('EndSinaisVitais', { cdPaciente: AgendaPaciente.cD_PESSOA_FISICA, Tipo: 'Todos' });
        break;
      case 'Acompanhantes':
        navigation.navigate('AcompanhateSinaisVitais', {
          PessoaFisica: {
            nM_PESSOA_FISICA: AgendaPaciente.nM_PESSOA_FISICA,
            dT_NASCIMENTO: AgendaPaciente.dT_NASCIMENTO,
            cD_PESSOA_FISICA: AgendaPaciente.cD_PESSOA_FISICA,
          },
        });
        break;
      case 'Observações':
  ...
  useEffect(() => {
    if (SinaisVitais) {
      ...
      GetSinaisVitais(AgendaPaciente.cD_PESSOA_FISICA)
        .then(response => {
  ...
  const renderItemSinaisVitais: ListRenderItem<PropsPage> = ({
    ...
    switch (Name) {
      ...
      case 'Escala Flebite':
        return <OptionEscalaFlebite nR_ATENDIMENTO={159969} />;
      default:
        return <Text></Text>;
    }
  };
  ...
  return (
    <View style={styles.container}>
      <View style={styles.MenuOptions}>
        <ModalAlertPaciente
          codPacient={AgendaPaciente.cD_PESSOA_FISICA}
        />
        <MenuPopUp
      ...
      <ModalCentralize style={{ width: RFPercentage(30), height: RFPercentage(30), justifyContent: 'space-around' }} ref={refModalCentralizeSenha}>
        <BtnCentered labelBtn='Normal' SizeText={18} onPress={() => gerarSenha(AgendaPaciente.seQ_FILAS_SENHA[0])} enabled={true} />
        <BtnCentered labelBtn='Prioridade' SizeText={18} onPress={() => gerarSenha(AgendaPaciente.seQ_FILAS_SENHA[1])} enabled={true} />
      </ModalCentralize>
      <NotificationSimple
...
export default UpdateSinaisVitais;
