        ...
        <View
          ...
          <View style={styles.box2}>
            ...
            <View style={styles.item}>
              ...
              <Text style={styles.text}>{`${moment(item.dT_SINAL_VITAL).format(
                'DD-MM-YYYY [às] HH:mm',
              )}`}</Text>
            </View>
            <View style={styles.item}>
          ...
          <View style={styles.box3}>
            <MenuPopUp
              ...
              onpress={(label) => {
                refMenuPopUp.current?.hideMenu(), MenuPopUpOptions(label, item);
              }}
            />
          </View>
        </View>
  ...
  const selectedOptions = (item: string) => {
    switch (item) {
      ...
      case 'Todos':
        setFilterOptions({
          cd_pessoa_fisica: usertasy.cD_PESSOA_FISICA,
          dataInicio: moment().subtract(1, 'year').format('YYYY-MM-DD'),
          dataFinal: moment().format('YYYY-MM-DD'),
        });
    ...
    <View style={styles.container}>
      ...
      {!isLoading ? (
        <FlatList
          ...
          renderItem={({ item, index }) => renderItemCall({ item, index })}
      ...
      <ModalFiltroData
        ...
        onPress={(initialDate, endDate) => {
          setFilterOptions({
            cd_pessoa_fisica: usertasy.cD_PESSOA_FISICA,
            dataInicio: moment(initialDate, 'DD/MM/YYYY').format('YYYY-MM-DD'),
            dataFinal: moment(endDate, 'DD/MM/YYYY').format('YYYY-MM-DD'),
          });
          ModalFiltroDataRef.current?.closeModal();
        }}
      />
