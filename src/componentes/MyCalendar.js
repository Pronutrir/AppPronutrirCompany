...
export default function MyModalCalendar({
  scrollToIndex,
  selectedHorizontal,
  medicamentos,
}) {
  ...
  const markedDatesFunc = date => {
    ...
    function diasSemana(params) {
      var dia =
        params.dS_INTERVALO_ITEM &&
        params.dS_INTERVALO_ITEM.toLocaleLowerCase();
    ...
    function verificaMedicamento(params) {
      ...
      var fim = params.dT_FIM
        ? moment(params.dT_FIM).format('YYYY-MM-DD')
  ...
  return (
    <View style={styles.container}>
      ...
      )}
      {
        <TouchableOpacity
          style={styles.btn3}
          onPress={() => navigation.navigate('MedicamentosAtivos')}>
