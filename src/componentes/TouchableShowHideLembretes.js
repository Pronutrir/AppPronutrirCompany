...
export default function TouchableShowHideLembretes({
    selectedIntervalo,
    ...
    const optionsIntervalo = [
        ...
        {
            id: '2',
            name: 'A CADA DUAS HORAS (2/2HS)',
            date: gerarHoras(2),
            intervalo: 2,
        },
        {
            id: '3',
            name: 'A CADA TRÊS HORAS (3/3H)',
            date: gerarHoras(3),
            intervalo: 3,
        },
    ...
    const addIntervalo = (intervalo) => {
        setSelectedIntervalo({
            ...intervalo,
            valueDose: 1,
            tipoDose: { id: 'com', name: 'Comprimido(s)' },
        });
    };
    ...
    const updateHorarios = (item) => {
        ...
        if (selectedIntervalo.intervalo && item.Date.id == 0) {
            selectedIntervalo.date = updateHoras(
                selectedIntervalo.intervalo,
                item.Date.hora,
                selectedIntervalo.date[indice],
            );
        } else if (selectedIntervalo.intervalo && item.Date.id == indice) {
            selectedIntervalo.date = updateHoras(
                selectedIntervalo.intervalo,
                selectedIntervalo.date[0],
                item.Date.hora,
            );
        } else {
        ...
        setSelectedIntervalo({
            ...selectedIntervalo,
            date: selectedIntervalo.date,
            tipoDose: item.TipoDose,
            valueDose: item.ValueDose,
            flag: false,
        });
    };
    ...
    const IntervaloMedi = ({ item }) => (
        <TouchableOpacity
            ...
            style={styles.btnIntervalo}
            onPress={() => itemSelected(item)}>
            <View>
                <Text style={styles.text}>
                    {moment(item.hora).format('HH:mm')}
                </Text>
                {item.id == 0 && item._length > 1 && (
                    <Text style={styles.textInfo}>Hora inicial</Text>
                )}
                {item.id == item._length - 1 && item._length > 1 && (
                    <Text style={styles.textInfo}>Hora final</Text>
                )}
            </View>
            <Text style={styles.text}>
                Tome {item.valueDose} {item.tipoDose.name}
            </Text>
        </TouchableOpacity>
    );
                                ...
                                return (
                                    <IntervaloMedi
                                        item={{
                                            ...
                                            id: index,
                                            tipoDose:
                                                selectedIntervalo.tipoDose,
                                            valueDose:
                                                selectedIntervalo.valueDose,
                                            _length: arrayLength,
                                        }}
                                    />
                                );
                    ...
                    {selectedIntervalo.date && (
                        <View
                            ...
                            <Text style={styles.textInfo}>
                                toque na hora inicial ou hora final para
                                reprogramar os lembretes
                            </Text>
                        </View>
                    )}
                </View>
