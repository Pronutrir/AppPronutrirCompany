...
export default function MyModalDiasSemana({ activeModal, setActiveModal, action, diasSelected }) {
    ...
    const addDiaSemana = (isChecked, item) => {
        if(isChecked && !dias_semana.some(element => element.id == item.id)){
            setDiasSemana([...dias_semana, item]);
        }
        if(!isChecked){
            setDiasSemana(dias_semana.filter((element) => element.id != item.id));
        }
    }
                ...
                <View style={styles.centeredView}>
                    <View style={styles.modalView}>
                        <View style={styles.box}>
                            <Text style={styles.text}>Escolha os dias da semana</Text>
                        </View>
                        {
                            DiasSemana.map(item => (
                                <View key={item.id} style={{ width: 150 }}>
                                    <BouncyCheckbox
                                        ...
                                        iconStyle={{color: 'red'}}
                                        text={item.dia}
                                        isChecked={diasSelected && diasSelected.some(element => element.id == item.id)}
                                        onPress={(isChecked) => addDiaSemana(isChecked ,item)}
                                    />
                                </View>
                            ))
                        }
