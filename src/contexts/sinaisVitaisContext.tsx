...
export const SinaisVitaisProvider: React.FC = ({ children }) => {
    ...
    return Api.get<any, AxiosResponse<ResponseSVMG>>(
      ...
      .then(response => {
        ...
        if (result) {
          ...
          addAlert({
            message: 'Paciente não possui sinais vitais cadastrados!',
            status: 'info',
          });
        }
      })
  ...
  const AddSinaisVitaisAtendimento = async (atendimento: SinaisVitaisPost) => {
    try {
      const { result } = (
        await Api.post<ISinaisVitais, AxiosResponse<ResponseSVMG>>(
          'SinaisVitaisMonitoracaoGeral/PostSVMGComAtendimento',
          {
            ...
            iE_SITUACAO: sinaisVitaisDefault.iE_SITUACAO,
            dT_LIBERACAO: moment().format(),
            nM_USUARIO: usertasy.usuariO_FUNCIONARIO_PERFIL[0].nM_USUARIO,
            dS_OBSERVACAO: atendimento.dS_OBSERVACAO,
          },
        )
      ).data;
  ...
  const ValidationAutorizeEnfermagem = useCallback(() => {
    return PerfisSinaisVitaisEnfermagem?.some((element: IPerfisLiberados) => {
